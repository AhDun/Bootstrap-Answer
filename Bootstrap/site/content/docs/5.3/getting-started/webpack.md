...
touch src/index.html src/js/index.js src/scss/styles.scss webpack.config.js
```
...
3. **Next we load the CSS and import Bootstrap's JavaScript.** Add the following to `src/js/index.js` to load the CSS and import all of Bootstrap's JS. Popper will be imported automatically through Bootstrap.
...
After running `npm run build` again, there will be a new file `dist/main.css`, which will contain all of the CSS imported by `src/js/index.js`. If you view `dist/index.html` in your browser now, the style will be missing, as it is now in `dist/main.css`. You can include the generated CSS in `dist/index.html` like this:
